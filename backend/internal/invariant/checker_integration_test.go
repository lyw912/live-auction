...
func cleanupInvariantFixtures(t *testing.T, db *pgxpool.Pool) {
	...
	statements := []string{
		...
		`DELETE FROM realtime_stream_epochs WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM snapshot_rebuild_events WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM auction_system_messages WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM auction_risk_alerts WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM auction_ai_settings WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM auction_engine_checkpoints WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM redis_engine_settlements WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM max_bid_intents WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM auction_highlight_assets WHERE auction_id LIKE 'auc_inv_%' OR job_id IN (SELECT id FROM ai_generation_jobs WHERE auction_id LIKE 'auc_inv_%' OR room_id LIKE 'room_inv_%')`,
		`DELETE FROM bids WHERE auction_id LIKE 'auc_inv_%'`,
		`DELETE FROM orders WHERE auction_id LIKE 'auc_inv_%' OR id LIKE 'ord_inv_%'`,
		`DELETE FROM ai_generation_jobs WHERE auction_id LIKE 'auc_inv_%' OR room_id LIKE 'room_inv_%'`,
