...
| `S2-capacity-stair` | Find the single-auction bid capacity knee | independent-ECS k6 | `../load/s2-steady-soak.js`; `prepare-s2-capacity-stair-pressure.sh` | Two explicit profiles. `accepted`: attack profile is 50/s -> 100/s -> 200/s -> 400/s -> 600/s, 1s ramp plus 90s hold per target, `AMOUNT_MODE=fast_ladder`, `NOISE_PCT=0`, `USER_COUNT=MAX_VUS`, intended to keep prices increasing and stress accepted updates, Kafka, settlement, outbox, and fanout source. The 2026-06-04 600/s runs are CURRENT_FAILING end-to-end async backlog evidence. The 400/s run `s2-capacity-accepted-clean400-p1-ecs-20260604T181824` is higher-ceiling evidence with late-drain/verifier-cost caveats. Current judge-facing display pass is `s2-capacity-accepted-display200-p1-ecs-20260604T192002`: 50/s -> 100/s -> 150/s -> 200/s, 30s/stage, 15,525 decisions, k6 clean, full verifier PASS. `decision`: 100/s -> 200/s -> 400/s -> 600/s -> 1000/s, 1s ramp plus 2m hold per target, `AMOUNT_MODE=time_ladder`, `NOISE_PCT=20`, intentionally includes normal low/self/stale rejects and measures Redis decision throughput. Do not mix the two claims. | k6 exit 0, dropped iterations 0 for clean stair, p99/p99.9 by stage, delivered vs target rate, accepted/rejected distribution, Kafka/Redis/PG/outbox backlog slope, k6 host CPU/RSS/FD/TCP retransmission, and final verifier PASS |
...
| `S3-mixed-final-burst-smoke` | Small PTS harness/network validation before paid mixed run | PTS JMeter | `scenarios/s3-room-fanout/s3-mixed-smoke-30vu.jmx` | 30 VU, 1 IP, 1 minute, 100% sampling, upload `inputs/s1-s5/s3-mixed-smoke-30-sessions.csv` | split labels for join/ticket/handshake/first message/live fanout/read/bid; report-details for exact sampler counts |
| `S3-mixed-final-burst` | Final-window bid burst with online viewers and controlled readers | PTS JMeter | `scenarios/s3-room-fanout/s3-mixed-final-burst-4500vu.jmx` | 1000 bid + 3000 WS + 500 reader = 4500 VU, 9 IP, one single main Thread Group, upload `inputs/s1-s5/s3-mixed-final-burst-4500-sessions.csv`; set PTS `是否指定循环=是`, `循环次数=1`, and duration long enough for connect + burst + observe | `verify-s3-pts-evidence.sh` with expected counts; live fanout p99 <= 1000ms; first-message p99 <= 1000ms; server metrics for WS/bid/read counts |
| `S4-fault` | Fault resilience, RTO/RPO, no double settlement | local k6 + SIGKILL/Toxiproxy | `run-pts-1c-concurrent-fault.sh`; `scenarios/s4-fault-resilience/s4-concurrent-fault.js`; `tests/chaos/*` | default `L1F_PROFILE=rto`, 200 VU, 25s, 1s pacing, 5s fault window | fault gates, RTO, Redis pending/Kafka lag/settlement/outbox convergence |
...
1. `S1-burst`, then optional `S1-ladder`
2. `S4-fault` P0 set: Redis, settlement worker, PostgreSQL
3. `S2-long-soak`, `S2-convergence-drain`, `S2-capacity-stair`, and `S2-read-interference`
4. `S3-live-only-fanout`, then `S3-mixed-final-burst`
5. `S4-fault` P1/P2 set: Kafka, Redis flush, Redis+Kafka, Redis partial timeout through Toxiproxy
6. `S5-reconnect`
