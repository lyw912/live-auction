...
| `S2-capacity-stair` | Find the bid decision knee | k6 first, optional PTS RPS | `../load/s2-steady-soak.js`; optional `L2-protocol/pts-2p4-steady-interactive-auction.jmx` | 100/s -> 200/s -> 400/s -> 600/s -> 1000/s, 2-5 min per stage | p99/p99.9, delivered rate, dropped iterations, backlog slope |
| `S2-read-interference` | Reader RPS impact on bid path | PTS RPS or independent-ECS k6 | `../load/s2-read-interference.js`; `prepare-s2-read-interference-pressure.sh` | 100/s bid attempts plus 2000/s -> 5000/s -> 10000/s HTTP reads, 80/15/5 snapshot/leaderboard/my-bids mix; do not reuse S3 fanout samplers. 2026-06-04 10k, 4k, and 1500/1800/2000 profiles are CURRENT_FAILING bottleneck evidence. After the Redis TTL P0 and read-path P1 mitigation, rerun 1500/s -> 1800/s -> 2000/s with label `s2-read-display-postfix-*`. | bid p99 under read load, read p99 by route, DB pool wait, correctness and convergence |
