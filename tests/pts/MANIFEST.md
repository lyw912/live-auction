...
| `S2-long-soak` | Steady auction stability and no leak | independent-ECS k6 | `../load/s2-steady-soak.js` | 20/s -> 60/s -> 100/s, 30-60 min, report dropped iterations | k6 p99 <= 100ms, dropped iterations bounded, Grafana heap/goroutine/fd flat |
...
| `S2-read-interference` | Reader RPS impact on bid path | PTS RPS or independent-ECS k6 | current script TBD; do not reuse S3 fanout samplers | staged GET auction snapshot / leaderboard / my bid history RPS with 20-100/s bids | bid p99 under read load, read p99, DB pool wait, correctness and convergence |
