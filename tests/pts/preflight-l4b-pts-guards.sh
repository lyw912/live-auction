...
SQL
  ...
  printf 'P0\tkafka_bid_topic_replication_explicit\t%s\tbid ledger topic replication factor must be explicit; RF=1 is local-only evidence\n' "$([ "${bid_topic_replication:-0}" -ge 1 ] && echo PASS || echo FAIL)"
  printf 'P1\tkafka_bid_topic_production_rf3\t%s\tproduction broker-failure durability proof requires replication factor >= 3; local RF=1 can only prove functional fence behavior\n' "$([ "${bid_topic_replication:-0}" -ge 3 ] && echo PASS || echo FAIL)"
  printf 'P1\tkafka_bid_topic_production_min_isr2\t%s\tproduction broker-failure durability proof requires min.insync.replicas >= 2 with acks=all\n' "$([ "${bid_topic_min_isr:-0}" -ge 2 ] && echo PASS || echo FAIL)"
