...
L4B_PROFILE="${L4B_PROFILE:-pts-1b}" \
...
SELECT 'k6_bidder_' || generate_series(1, :'max_vus'::int)::text,
       'user',
       'k6_bidder_' || generate_series(1, :'max_vus'::int)::text,
       's2-capacity'
