...
if [ "${ALLOW_HISTORICAL_PTS:-}" != "1" ]; then
  echo "reset-accepted-pressure-data.sh is historical accepted-pressure setup." >&2
  echo "Use current PTS-1A/PTS-1B reset from tests/pts/MANIFEST.md instead." >&2
  echo "To run this archived script intentionally, set ALLOW_HISTORICAL_PTS=1." >&2
  exit 2
fi

export SESSION_COUNT="${SESSION_COUNT:-4096}"
export SESSION_CSV="${SESSION_CSV:-archive/data/pts_accepted_sessions.csv}"
export JMX_PATH="${JMX_PATH:-$ROOT_DIR/tests/pts/archive/historical/live-auction-accepted-pressure.jmx}"
...
echo "accepted pressure data reset complete"
echo "- JMX: tests/pts/archive/historical/live-auction-accepted-pressure.jmx"
