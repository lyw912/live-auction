...
PG_VERIFY_OPTIONS="${PG_VERIFY_OPTIONS:--c max_parallel_workers_per_gather=0 -c max_parallel_workers=0 -c enable_parallel_hash=off -c jit=off -c statement_timeout=120000}"
...

{
  ...
  set +e
  docker exec -e PGOPTIONS="$PG_VERIFY_OPTIONS" -i "$DB_CONTAINER" psql -U "$DB_USER" -d "$DB_NAME" \
    -v ON_ERROR_STOP=0 -v auction_id="$AUCTION_ID" -f - <<'SQL'
\pset pager off
\timing on
...
SQL
  detailed_status=$?
  set -e
  if [ "$detailed_status" -ne 0 ]; then
    echo
    echo "## detailed diagnostics incomplete"
    echo "best_effort_status=$detailed_status"
    echo "The detailed diagnostic section failed after some queries. Machine-readable P0 gates below remain authoritative for pass/fail."
  fi

  echo
  echo "## machine-readable invariant gates"
  set +e
  docker exec -e PGOPTIONS="$PG_VERIFY_OPTIONS" -i "$DB_CONTAINER" psql -q -A -F $'\t' -U "$DB_USER" -d "$DB_NAME" \
...
ordered_bid_prices as (
  select b.auction_id,
         ...
         b.reject_reason,
         b.amount_cents,
         max(b.amount_cents) filter (where b.status = 'ACCEPTED') over (
           partition by b.auction_id, b.engine_epoch
           order by b.engine_seq
...
low_price_reject_violations as (
  select count(*) as violations
  from ordered_bid_prices b
  join auctions a on a.id = b.auction_id
  where b.auction_id = :'auction_id'
    and b.status = 'REJECTED'
    and b.reject_reason = 'BID_TOO_LOW'
    and b.amount_cents >= coalesce(b.prior_price, a.start_price_cents) + a.increment_cents
),
...
SQL
  ...
  if [ "$machine_status" -ne 0 ]; then
    echo
    echo "## machine-readable invariant gate query incomplete"
    echo "best_effort_status=$machine_status"
    echo "The gate query returned a non-zero status. Any emitted PASS/FAIL rows are still extracted below; missing gate output fails the script."
  fi
...
awk -F '\t' '$1 ~ /^P[0-9]$/ && $3 ~ /^(PASS|FAIL)$/ { print }' \
  "$OUT_DIR/l4b-correctness.txt" > "$OUT_DIR/l4b-invariant-gates.tsv"
if [ ! -s "$OUT_DIR/l4b-invariant-gates.tsv" ]; then
  echo "[verify] failed to extract machine-readable gates from $OUT_DIR/l4b-correctness.txt" >&2
  exit 2
fi

required_invariant_gates="
auction_exists
engine_not_paused
no_non_terminal_settlements
redis_kafka_pg_accepted_match
auction_accepted_count_matches_pg
redis_engine_seq_matches_settlement
kafka_position_present
accepted_settlement_has_public_event
no_public_auction_event_seq_gap
no_duplicate_client_bid_id
no_duplicate_engine_seq
engine_epoch_seq_monotonic
kafka_offset_matches_engine_order
no_created_at_seq_inversion
no_accepted_after_final_end
increment_grid_valid
at_most_one_order
no_cross_auction_event_payload_leak
outbox_drained
auction_winner_matches_highest_accepted
engine_seq_complete
bid_too_low_rejects_justified
accepted_public_event_exact_mapping
public_events_have_published_outbox
every_bid_has_settled_ledger
idempotency_response_matches_bid
cap_terminal_single_sold_order
soft_close_no_stacked_subwindow_extension
"
for gate_name in $required_invariant_gates; do
  if ! awk -F '\t' -v gate="$gate_name" '$2 == gate { found=1 } END { exit found ? 0 : 1 }' "$OUT_DIR/l4b-invariant-gates.tsv"; then
    echo "[verify] missing required invariant gate '$gate_name'; machine-readable verifier output is incomplete" >&2
    exit 2
  fi
done
...
redis_pending_gate_file="$OUT_DIR/l4b-redis-pending-gates.tsv"
{
  ...
  settlement_total="$(docker exec -e PGOPTIONS="$PG_VERIFY_OPTIONS" -i "$DB_CONTAINER" psql -q -A -t -U "$DB_USER" -d "$DB_NAME" \
...
reject_reason_gate_file="$OUT_DIR/l4b-reject-reason-gates.tsv"
{
  ...
  docker exec -e PGOPTIONS="$PG_VERIFY_OPTIONS" -i "$DB_CONTAINER" psql -q -A -t -F $'\t' -U "$DB_USER" -d "$DB_NAME" \
    -v ON_ERROR_STOP=1 -v auction_id="$AUCTION_ID" -f - <<'SQL'
SET max_parallel_workers_per_gather = 0;
