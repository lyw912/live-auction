...
These runbooks cover local Prometheus alert rules. They are operational guardrails for diagnosis and demo evidence, not a production paging policy.
...
Escalate when anomaly type is `OUTBOX_DEAD_LETTER`, `ORDER_CREATE_FAILED`, `CLOCK_STEP_BACKWARD`, or `REDIS_DB_RECONCILIATION_DRIFT`.
...
Do not hand-edit Redis history or snapshot as a hidden fix.

## LiveAuctionOutboxLagHigh

Meaning:

- P95 `auction_outbox_lag_seconds` is above the local guard threshold.
...
- This is not a QPS or P99 capacity claim.
...
- P95 `auction_scheduler_drift_seconds` is above the local guard threshold for a scheduler job type.
...
- idempotent scheduler retry is allowed;
- auction truth remains in PostgreSQL.
...
- H5 CTA stays disabled during recovering or disconnected states.
- Realtime quality is not claimed during Redis outage.

## LiveAuctionSnapshotRebuildPressure

Meaning:

- `auction_snapshot_source_total` shows pressure on DB, stale, or unavailable snapshot recovery paths.

First checks:

1. Check Redis history and snapshot availability.
...
Do not bypass recovery by trusting client-side state.
...
4. Use slow-consumer k6 or Playwright evidence before making capacity claims.
