...
```text
1. reads current auction snapshot using a real Bearer token from the session CSV
   generated by the PTS reset script
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| `s5-20260604T221312` | clean close | 200 | 2m | 10/s, 5 VU | 34,814 | 87 ms | 0 | 0 | 0 | **PASS** | external run artifact |
| `s5-20260604T231925` | initial online WS clean, reconnect leg through Toxiproxy `reset_peer` at `ws://127.0.0.1:18081` | 50 | 2m | 10/s, 5 VU | 8,849 | 341 ms | 0 | 0 | 0 | **PASS** | external run artifact |
...
| `s5-clean-20vu-20260603T0254` | k6 could not open session CSV | k6 `open()` path was relative to script directory | session CSV path now comes from reset output/run env |
