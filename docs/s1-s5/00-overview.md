...
| **Few, precise metrics.** 5 headline numbers total; each maps to exactly one named PTS sampler or one Grafana panel. | Skeptical judges interrogate every number; a wall of metrics dilutes the ones that matter. | [metrics-and-slo.md](01-metrics-and-slo.md) |
...
| **Per-shard capacity + scale-out narrative.** Report single-node numbers as "one shard's worth" and name the horizontal path. | Only the *per-auction single sequencer* is a true architecture ceiling; everything else (Redis pub/sub, Kafka partitions, WS gateways) scales by sharding. | [scale-out-and-architecture-ceilings.md](08-scale-out-and-ceilings.md) |
| **Production HA claims stay falsifiable.** S4/S5 local pass is not presented as Kafka RF=3 / Redis HA / multi-gateway proof; those are separate topology tests with quorum, fencing, LB/NAT, mobile, and AZ evidence gates. | Judges will attack the gap between local chaos and production incidents. The answer must be a concrete failure contract and next-test matrix, not "add HA." | [production-ha-expansion-and-judge-defense.md](09-production-ha-judge-defense.md) |
| **PTS for charts, local k6 for everything cheap.** PTS earns its cost only where distributed source IPs + clean per-sampler p99 export add judge value. | PTS bills `⌈maxVU/500⌉×500×min×(1+sample%)`; soak/fault/reconnect need neither. | [pts-playbook.md](13-pts-playbook.md) |
...
Per-scenario specs: [s1](02-s1-final-second-contention.md) · [s2](03-s2-steady-auction-and-soak.md) ·
[s3](04-s3-room-fanout.md) · [s4](05-s4-fault-resilience.md) · [s5](06-s5-reconnect-recovery.md).
...
| 技术选型适配高并发直播竞拍 | 25% | **S1** (Redis single-writer) + **S3** (room-isolated WS) — see [scale-out](08-scale-out-and-ceilings.md) |
...
| File | Purpose |
|---|---|
| [`00-overview.md`](00-overview.md) | spine: scenarios, metrics map, cost, staging, reading order |
| [`01-metrics-and-slo.md`](01-metrics-and-slo.md) | the 5 headline metrics, exact definitions, SLO targets, what is *not* a metric |
| [`02-s1-final-second-contention.md`](02-s1-final-second-contention.md) | 绝杀: burst vs ladder, script logic, PTS config, metric to chart |
| [`03-s2-steady-auction-and-soak.md`](03-s2-steady-auction-and-soak.md) | 稳态: open-model arrival, realistic bid mix, leak detection |
| [`04-s3-room-fanout.md`](04-s3-room-fanout.md) | 围观: fanout latency measurement, 10k headline + cost variant, RAM/conn |
| [`05-s4-fault-resilience.md`](05-s4-fault-resilience.md) | 故障: chaos structure, minimal fault set, RTO/RPO, no-double-charge test |
| [`06-s5-reconnect-recovery.md`](06-s5-reconnect-recovery.md) | 断连重连: time-to-current-state |
| [`07-s2-s3-expanded-design.md`](07-s2-s3-expanded-design.md) | S2/S3 split into long soak, convergence, capacity, read interference, live-only fanout, and mixed final burst |
| [`08-scale-out-and-ceilings.md`](08-scale-out-and-ceilings.md) | infra-vs-architecture boundary, per-shard framing, judge Q&A prep |
| [`09-production-ha-judge-defense.md`](09-production-ha-judge-defense.md) | Kafka RF=3/minISR=2, Redis HA failover, multi-gateway, LB/NAT, mobile weak network, and cross-AZ/region judge-defense expansion plan |
| [`10-fault-injection-runbook.md`](10-fault-injection-runbook.md) | local k6 + Toxiproxy fault runbook |
| [`11-fault-test-matrix.md`](11-fault-test-matrix.md) | fault matrix and pass/fail interpretation |
| [`12-readiness-checklist.md`](12-readiness-checklist.md) | pre-run and post-run checklist |
| [`13-pts-playbook.md`](13-pts-playbook.md) | PTS sampler naming, upload, reporting, and pitfalls |
...
- **Implementer (another model/engineer building scripts):** this overview → the
  target `0N-sN-*.md` → `13-pts-playbook.md`.
- **Reviewer writing the judge report:** `01-metrics-and-slo.md` → each `S1-S5`
  "what to show" section → `../judge/02-s1-s5-gate-mapping.md`.
