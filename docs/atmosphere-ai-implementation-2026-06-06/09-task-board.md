...
| P1-07 | Done with limits: add provider-backed AI commentator with deterministic fallback and automatic decided-event commentary. | Backend | Bid gateway now creates non-blocking background commentary after accepted/sold decisions; `CreateAutoCommentary` uses the configured provider boundary, dedupes by auction/source seq, persists an `auction_commentary` job, and marks `auto_generated`. Targeted gateway tests cover provider output and fallback. This is not a separate durable event-consumer service. |
...
- Buyer/merchant UX language pass is now implemented for the normal H5 buyer route and PC merchant workbench route: H5 no longer surfaces `WebSocket`, `seq`, `Max Bid`, `ORDER`, raw terminal/status words, or room IDs in the primary experience; PC merchant queues/rules/live-assist use `待完善/已排期/开拍中/已结束`, `直播助手`, `自动加价概况`, `近30秒热度`, and `事件回放` instead of English engineering labels. Technical fields remain visible only in diagnostics/audit contexts.
