...
| P2-04 | Done: add auction recap/highlight generator. | Backend/PC/H5 | Backend/PC recap remains host-only and now persists a server-side highlight job/asset (`auction_highlight_assets`) as a server-rendered HTML reel with masked/public facts; PC opens/downloads the asset. H5 renders a buyer-safe result recap/share card from public current-state facts only, with real copy, downloadable SVG highlight card, and browser-generated WebM highlight video. This is a real server highlight pipeline, but not an MP4/WebM transcoding farm or licensed motion template pack. |
...
- P1-E victory ceremony is now a full-screen H5 result ceremony with recap ticket, next action, downloadable browser-generated WebM highlight video, and a backend/PC server highlight asset pipeline. Remaining limit: current server artifact is HTML reel output, not MP4/WebM transcoding.
...
- B4 Recap includes backend/PC host recap, persisted server highlight assets, PC open/download actions, H5 buyer-safe share/highlight card, copy action, downloadable SVG highlight card, and browser-generated WebM highlight video. Remaining limit: no MP4/WebM server transcoding service or licensed motion template pack.
