...
| P2-06 | Done with limits: add compliant warm-up, buyer PK, entry/leader effects. | Backend/H5 | Backend `liveops_campaigns`, `liveops_task_progress`, and `liveops_team_choices` persist room campaign state, per-user task completion, and buyer-team selection/counts. H5 `LiveOpsPanel` reads server progress and only marks task completion after the API returns; buttons open real sheets, complete real task APIs, or persist buyer-team choice. Copy explicitly says no lottery, no promised reward, no price/winner impact. This is not a random 福袋, cash/reward campaign, or price/winner-affecting promotion system. |
