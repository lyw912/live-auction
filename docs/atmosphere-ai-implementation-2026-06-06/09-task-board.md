...
| P0-05 | Done with limits: add final-countdown visual/audio tension gated by state and reduced-motion. | H5 CSS/components | Countdown phase drives `data-countdown-phase`; hot/critical/hammer styles are state-gated, hammer beat shows `第一次/第二次/最后一次` with server-result copy, and opt-in audio/haptics fire only in active connected state. H5 now loads a project-owned generated WAV sound pack with heartbeat bed, hammer, whoosh, coin, and chime assets, with WebAudio fallback. It is still not a third-party licensed professional soundbed. |
...
- Product UX refactor evidence: `pnpm --filter mobile-h5 build`, `pnpm --filter pc-console build`, and `H5_PORT=5276 PC_PORT=5277 PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 pnpm exec playwright test tests/e2e/mobile-h5.spec.ts tests/e2e/pc-console.spec.ts --project=mobile-h5 --project=pc-console --reporter=line -g 'feed product card|AI|recap|leaderboard'` passed on 2026-06-06 after the Chinese/business-language UI refactor.
...
| P1-02 | Done: add winner/loser/unsold cinematic ceremony variants and server highlight artifact. | H5/Backend/PC | `ResultSheet` has full-screen winner, loser, and unsold variants with cinematic background glow, capped confetti, recap ticket, next-action copy, and compact fallback inside the bid panel. Backend recap creates persisted `auction_highlight_assets` with a server-rendered WebM reel asset from ffmpeg, and PC exposes real open/download actions. Rendering is a host recap action, one render runs per process, recent same-auction assets are reused, and it never blocks bid/order paths. |
...
| P1-04 | Done: add measured FLIP rank transition animation and WS leaderboard delta. | Backend/H5 | `LeaderboardRows` measures previous/current row positions and animates rank movement with Web Animations; reduced-motion disables transitions. Stable row dimensions prevent layout shift. Outbox relay now enqueues non-critical `leaderboard_delta` projection work after durable accepted/sold/ended events into a bounded async queue; queue saturation skips the projection rather than blocking auction-event fanout. H5 applies latest-wins deltas without REST polling, coalesces burst deltas into one repaint, rejects stale REST leaderboard responses, suppresses noisy burst sound/animation, and keeps manual REST refresh for recovery. |
| P1-05 | Done with limits: add opt-in layered sound, countdown haptics, and browser TTS. | H5 | `loadAuctionSoundPack`, `playAuctionSound`, `playCountdownTone`, `playLayeredCue`, `vibrateCountdownPhase`, and `speakSystemMessage` run only after the user enables sound, while visible and supported. Generated WAV assets live under `frontend/mobile-h5/public/audio/auction` and now cover heartbeat, hammer, rank, leading, system message, 福袋开奖, 入场牌, and PK surge cues; WebAudio synthesis remains fallback. It is still not a licensed third-party professional sound asset pack. |
...
| P1-07 | Done: add provider-backed AI commentator with deterministic fallback and durable automatic decided-event commentary. | Backend | Bid gateway now only enqueues automatic commentary off the bid response path. `ai_generation_jobs` stores PENDING auto-commentary jobs with input-hash dedupe, worker lease, attempt count, and retry/failure state; the embedded worker claims jobs with `FOR UPDATE SKIP LOCKED`, uses configurable batch size and per-task timeout, generates through the configured provider boundary, persists `auction_system_messages`, and backfills missed `bid_accepted`/`auction_sold` events from `auction_events` using event payload facts. Targeted gateway tests cover provider output, fallback, timeout/failure, dedupe, durable queue processing, and event backfill. |
...
| P2-04 | Done: add auction recap/highlight generator. | Backend/PC/H5 | Backend/PC recap remains host-only and now persists a server-side WebM highlight job/asset (`auction_highlight_assets`) rendered by ffmpeg from masked/public facts; PC opens/downloads the `.webm` asset. H5 renders a buyer-safe result recap/share card from public current-state facts only, with real copy, downloadable SVG highlight card, and browser-generated WebM highlight video. This is a real server highlight pipeline, but not a distributed transcoding farm or licensed motion template pack. |
| P2-05 | Done: add provider-backed multi-turn buyer product Q&A from approved listing facts. | Backend/H5 | H5 Q&A now keeps a compact room-thread conversation, sends recent turns with each follow-up, and renders buyer-facing answer history plus real follow-up buttons. Backend sends only whitelisted item/rule facts and sanitized recent turns to the provider, validates `facts_used` against that whitelist, rejects unsafe authenticity/investment/private-bid claims back to fallback, persists each `product_qa` job with `thread_id`/`recent_turns`, and returns buyer-safe answer copy plus follow-up prompts. |
| P2-06 | Done with limits: add warm-up, 福袋, buyer PK, entry/leader effects. | Backend/H5 | Backend `liveops_campaigns`, `liveops_task_progress`, `liveops_team_choices`, and `liveops_lucky_draw_entries` persist room campaign state, per-user task completion, buyer-team selection/counts, 福袋 participation, and deterministic reward reveal. H5 `LiveOpsPanel` reads server progress and only marks task/福袋 state after APIs return; buttons open real sheets, complete task APIs, persist buyer-team choice, enter 福袋, and open rewards. Rewards are demo interaction rewards and do not affect price/winner. |

## Review Design Cross-Check

Review-design cross-check against `docs/reviews/extreme-bidding-atmosphere-and-ai-judge-review-and-design-2026-06-05.md`; remaining boundaries are explicit:

- P0-A is now covered for state-gated hot/critical/hammer visuals, `第一次/第二次/最后一次` beat copy, generated heartbeat/hammer/whoosh/coin/chime WAV assets, opt-in WebAudio fallback, and haptics. It still does not include a third-party licensed professional heartbeat soundbed.
- P1-E victory ceremony is now a full-screen H5 result ceremony with recap ticket, next action, downloadable browser-generated WebM highlight video, and a backend/PC server WebM highlight asset pipeline. Rendering is host-triggered, per-process concurrency-limited, and recent same-auction assets are reused. Remaining limit: this is a single-node ffmpeg render path, not a distributed transcoding farm or licensed motion template pack.
- P1-F leaderboard 2.0 now covers bid-count, rank labels, measured FLIP transitions, WS `leaderboard_delta`, bounded async projection queue, queue-full skip behavior, H5 latest-wins/rAF burst coalescing, stale REST overwrite protection, and no REST polling after WS deltas. Ranking projection is deliberately non-critical and asynchronous to protect bid p99 under final-second bursts.
- P1-G sound design 2.0 now includes an opt-in generated WAV asset pack, heartbeat bed loop, event sounds, 福袋/入场/PK cues, haptics, WebAudio fallback, and browser TTS for system messages; no third-party licensed professional asset pack.
...
- P2-I/P2-J/P2-K are implemented as a backend/H5 live-ops engine with persisted task progress, 福袋 participation/reward reveal, persisted buyer-team choice/counts, and entry/leader effects. Rewards are demo interaction rewards and do not affect price or winner.
- Buyer/merchant UX language pass is now implemented for the normal H5 buyer route and PC merchant workbench route: H5 no longer surfaces `WebSocket`, `seq`, `Max Bid`, `ORDER`, raw terminal/status words, or room IDs in the primary experience; PC merchant queues/rules/live-assist/order detail/event playback use `待完善/已排期/开拍中/已结束`, `直播助手`, `自动加价概况`, `近30秒热度`, `支付编号`, and `事件回放` instead of English engineering labels. Technical fields remain visible only in diagnostics/audit raw tables.
...
- B2 AI commentary is automatic for decided bid/sold events and host-triggered for manual prompts; both paths can use the external AI provider when configured, with deterministic fallback on missing/failed provider. Automatic commentary is now durable: accepted/sold events enqueue PENDING jobs, an embedded worker processes them with lease/retry semantics, and a recent-event backfill scans authoritative `auction_events` so a transient enqueue miss does not silently drop commentary.
...
- B4 Recap includes backend/PC host recap, persisted server WebM highlight assets, PC open/download actions, H5 buyer-safe share/highlight card, copy action, downloadable SVG highlight card, and browser-generated WebM highlight video. Remaining limit: no distributed MP4/WebM transcoding farm or licensed motion template pack.
- B5 Q&A now supports provider-backed multi-turn room Q&A constrained to approved listing/rule facts, with deterministic fallback and H5 answer history. It is still not a personalized recommendation engine, after-sales客服, order dispute agent, or human handoff/ticketing system.
...
- Production-money 福袋, cash rewards, coupons with settlement value, or price/winner-affecting promotions before campaign rules, odds disclosure, anti-addiction copy, and platform compliance review are written.
