...
| P1-02 | Done: add winner/loser/unsold cinematic ceremony variants and server highlight artifact. | H5/Backend/PC | `ResultSheet` has full-screen winner, loser, and unsold variants with cinematic background glow, capped confetti, recap ticket, next-action copy, and compact fallback inside the bid panel. Backend recap now also creates persisted `auction_highlight_assets` with a server-rendered WebM reel asset from ffmpeg, and PC exposes real open/download actions. Rendering is asynchronous from host recap and never blocks bid/order paths. |
...
| P1-04 | Done: add measured FLIP rank transition animation and WS leaderboard delta. | Backend/H5 | `LeaderboardRows` measures previous/current row positions and animates rank movement with Web Animations; reduced-motion disables transitions. Stable row dimensions prevent layout shift. Outbox relay now publishes non-critical `leaderboard_delta` messages after durable accepted/sold/ended events; H5 applies latest-wins deltas without REST polling and keeps manual REST refresh for recovery. |
...
| P1-07 | Done: add provider-backed AI commentator with deterministic fallback and durable automatic decided-event commentary. | Backend | Bid gateway now only enqueues automatic commentary off the bid response path. `ai_generation_jobs` stores PENDING auto-commentary jobs with input-hash dedupe, worker lease, attempt count, and retry/failure state; the embedded worker claims jobs with `FOR UPDATE SKIP LOCKED`, generates through the configured provider boundary, persists `auction_system_messages`, and backfills missed `bid_accepted`/`auction_sold` events from `auction_events` using event payload facts. Targeted gateway tests cover provider output, fallback, dedupe, durable queue processing, and event backfill. |
...
- P1-E victory ceremony is now a full-screen H5 result ceremony with recap ticket, next action, downloadable browser-generated WebM highlight video, and a backend/PC server WebM highlight asset pipeline. Remaining limit: this is a single-node ffmpeg render for host recap, not a distributed transcoding farm or licensed motion template pack.
- P1-F leaderboard 2.0 now covers bid-count, rank labels, measured FLIP transitions, and WS `leaderboard_delta` from the outbox relay. Ranking projection is deliberately non-critical and asynchronous to protect bid p99 under final-second bursts.
