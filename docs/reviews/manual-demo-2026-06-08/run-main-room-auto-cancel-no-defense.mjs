...
const outDir = 'docs/reviews/manual-demo-2026-06-08/evidence/main-room-auto-cancel-no-defense';
...
async function waitForCancelledIntent(cookie) {
  let last = null;
  for (let i = 0; i < 80; i += 1) {
    last = await getTruth(cookie);
    if (last.intent?.status === 'CANCELLED') return last;
    await new Promise((resolve) => setTimeout(resolve, 120));
  }
  throw new Error(`max bid intent did not cancel: ${JSON.stringify(last?.intent)}`);
}
...
async function closeH5Sheet(page) {
  await page.getByRole('button', { name: '关闭面板' }).click();
  await page.getByTestId('bottom-sheet').waitFor({ state: 'detached', timeout: 8000 }).catch(() => {});
}
...
async function setH5MaxBid(page, amountYuanText, screenshotName) {
  ...
  await sheet.getByText(/自动加价已生效|自动加价已防守|自动加价已被超越/).waitFor({ timeout: 12000 });
  await closeH5Sheet(page);
}
...
async function cancelH5MaxBid(page) {
  ...
  await sheet.getByText('未启用').waitFor({ timeout: 12000 });
  await page.screenshot({ path: path.join(outDir, '04-h5-auto-cancelled-confirmed.png'), fullPage: true });
  await closeH5Sheet(page);
}
...
async function main() {
  ...
  evidence.push({ step: 'h5_auto_defends_500', truth: await waitForTruth(buyer.cookie, { price: 50000, winner: 'user_1' }) });
  await h5.getByLabel('auction-state').getByRole('heading', { name: '¥500.00' }).waitFor({ timeout: 12000 });
  await h5.screenshot({ path: path.join(outDir, '03b-h5-auto-defense-500.png'), fullPage: true });

  await cancelH5MaxBid(h5);
  evidence.push({ step: 'h5_cancel_auto_confirmed', truth: await waitForCancelledIntent(buyer.cookie) });

  await pcDemoClick(pc, '第三方强挑战', '¥600.00');
  const finalTruth = await waitForTruth(buyer.cookie, { price: 60000, winner: 'user_3', status: 'SOLD' });
  if (finalTruth.intent?.status !== 'CANCELLED') {
    throw new Error(`cancelled intent became active again: ${JSON.stringify(finalTruth.intent)}`);
  }
  const bodyText = await h5.locator('body').innerText();
  if (bodyText.includes('自动加价防守到 ¥600.00') || bodyText.includes('自动加价已防守到 ¥600.00')) {
    throw new Error('H5 still displayed an auto defense after cancellation');
  }
  await h5.screenshot({ path: path.join(outDir, '05-h5-after-pc-challenge-no-defense.png'), fullPage: true });
  await pc.screenshot({ path: path.join(outDir, '05-pc-after-pc-challenge-no-defense.png'), fullPage: true });
  evidence.push({ step: 'pc_challenge_after_cancel_user3_sold', truth: finalTruth });
