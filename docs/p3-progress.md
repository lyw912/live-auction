...
| P3-02 | Relay shard ownership for multi-instance backend | IMPLEMENTED_WITH_LOCAL_GATE | `docs/evidence/p3-02-relay-shard-ownership-2026-05-24.md` records DB shard lease ownership and focused failover test. Two-process owner-kill pressure evidence is still required before claiming multi-instance realtime. |
...
- 2026-05-24 P3-02 added DB shard leases for outbox relay workers. This prevents live duplicate owners for a shard and allows takeover after lease expiry. It is a local correctness gate only; the next attack must run two backend processes and kill the owner during burst before upgrading the status to DONE.
