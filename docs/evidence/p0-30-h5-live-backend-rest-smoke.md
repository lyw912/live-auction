...
This slice extends the live smoke beyond WebSocket connect/fanout. The browser and Playwright API client call real backend REST endpoints for room auctions, snapshot, bid, bid history, order history, and mock payment.
...
- Browser rendered bid history row `¥400.00 · ACCEPTED`.
...
- Live fat-finger confirm is not covered here because the backend confirm-token pipeline and `/api/auctions/{id}/bids/confirm` route are not implemented yet. Existing H5 confirm coverage remains a mocked UI contract test.
...
Implement backend fat-finger confirm token persistence and route, then add live confirm smoke. After that, replace scaffold H5 IDs with room auction selection from `/api/rooms/{room_id}/auctions`.
