...
Command: `pnpm build`; `pnpm test:e2e`; `pnpm test:e2e:h5-live`; `go test ./...` from `backend`
...
| Bid truth path | accepted bid, rejected bid, cap SOLD order, payment double click, idempotency, backend fat-finger confirm, H5 live REST bid/confirm/cap-SOLD/order/payment/history smoke | `p0-06-p0-07-bid-truth-path.md`, `p0-21-h5-payment-double-click.md`, `p0-24-h5-fat-finger-confirm.md`, `p0-30-h5-live-backend-rest-smoke.md` plus current live smoke |
...
| Diagnostics | real monitor diagnostic producers, host ACL, recent rejects, recovery source mix | `p0-14-monitor-diagnostics-apis.md` plus current code/tests |
...
| H5 UI | test-only state matrix, normal entry without state matrix, pending/rejected bid, recovering CTA disable, server-driven SOLD winner/loser, payment double-click, fat-finger confirm UI contract, bid/order history, live backend cap-SOLD/order/payment smoke, room auction selection and current-auction payment target, chat seed/send, animation longtask smoke | `p0-18-frontend-state-surfaces.md`, `p0-20-h5-bid-protocol.md`, `p0-21-h5-payment-double-click.md`, `p0-22-h5-snapshot-recovery.md`, `p0-24-h5-fat-finger-confirm.md`, `p0-25-h5-history-ui.md`, `p0-30-h5-live-backend-rest-smoke.md`, `p0-33-animation-longtask.md` plus current code/tests |
| PC UI | item/create auction flow, selected-auction rule save, lifecycle controls, order list, diagnostics tabs, live browser-to-backend host smoke | `p0-19-pc-rule-validation.md`, `p0-23-pc-rule-save.md`, `p0-26-pc-full-rule-fields.md`, `p0-35-pc-live-backend-host-smoke.md` |
...
| Freeze review and repair | final P0 review, outbox poison gap notice fix, expired relay lease recovery, order paid/expired realtime events, P1 entry verdict | `p0-34-freeze-review.md`, `p0-36-p0-repair-and-p1-entry-review.md` |
...
- H5 still enters the deterministic local room `room_main`; auction and payment order IDs are selected from live API responses, and live smoke now proves cap SOLD creates the payable order before payment.
- Demo flow and known limits are recorded under `docs/demo/`.

## Next Action

P0 is ready for P1 with documented risks after the repair record in `p0-36-p0-repair-and-p1-entry-review.md`. Only run final performance baselines if the project wants to publish P99/QPS/fanout numbers.
