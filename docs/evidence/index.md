...
| `docs/adr/p3-03-rejected-bid-realtime-policy.md` | AUTHORITATIVE | Decision to keep ordinary non-state bid rejects out of full-room durable realtime while preserving HTTP idempotency, bid-row audit, monitor rejects, and flight-recorder diagnostics. |
| `docs/evidence/p3-18-rejected-bid-realtime-policy.md` | AUTHORITATIVE_FOR_IMPLEMENTATION | P3-18 implements the rejected-bid realtime policy for ordinary rejects; cloud performance impact remains unproven until fresh ECS/PTS evidence. |
...
| `docs/evidence/p10-02-ui-performance-gate.md` | AUTHORITATIVE_FOR_UI_CONTRACT | P10-S2 adds a Windows-local Playwright UI performance gate for rapid H5 bid-event rendering, longtasks, frame gaps, layout shift, and Bid CTA stability; not backend capacity evidence. |
| `docs/evidence/p10-03-no-mock-auction-demo.md` | AUTHORITATIVE_FOR_DEMO_TRUNK | P10-S3/S4 separates route-mocked UI coverage from the judge-facing no-mock trunk: live smoke creates item/auction, schedules/starts, connects H5 WebSocket, bids through backend APIs, reaches SOLD, and captures the created auction's flight-recorder path. Latest update also documents the feed-first H5 entry, terminal-card non-overlap, clickable PC nav, capped history queue, seeded product image, and local/test host-only competing-bid driver. |
| `docs/evidence/pts-l1-admission-debounce-2026-05-28.md` | AUTHORITATIVE | PTS L1 admission/debounce implemented from `1d31bf9`: completed bid replay bypasses admission, Redis/local rate limits return `Retry-After`, and H5 pending/cooldown behavior is covered by focused backend and Playwright tests. |
...
| `docs/evidence/pts-l4b-redis-ledger-engine-2026-05-29.md` | AUTHORITATIVE | PTS L4b implements the hot engine as Redis Lua state machine + Kafka/Redpanda durable ledger + PostgreSQL settlement truth, with engine epoch/seq fencing, bounded settlement retry, DLQ pause, reconciliation, and invariant verifier checks. No PTS latency number is claimed yet. |
