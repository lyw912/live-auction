...
> 2026-05-31 supersession notice: this is a historical P3/P4 decision log. It is no longer the current decision surface for hot bidding. In particular, decisions that keep PostgreSQL as synchronous bid truth or Redis as projection-only are superseded by `docs/current/architecture.md` and `docs/current/performance-correctness-contract.md`. Use this file only for historical rationale unless a current doc explicitly revalidates a decision.

> Date: 2026-05-24 Asia/Shanghai
> Original status: authoritative P3 decision surface after roadmap reset.
> Current status: historical decision record; see `docs/archive/progress-history-map.md`.
