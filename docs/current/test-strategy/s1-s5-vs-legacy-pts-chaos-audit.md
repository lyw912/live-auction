...
For the concrete "did pressure really arrive, and what exactly did the gates
check?" audit, use
[`s1-s5-pressure-and-gate-audit.md`](s1-s5-pressure-and-gate-audit.md). That
document maps each S1-S5 PASS to load evidence, service-side counters, durable
state, and user-visible correctness gates.
...
|---|---|---|---|---|---|---|---|
| S1 final-second contention | 1000 PTS users, one hot auction, one bid each; current formal JMX default uses `contention_release_window_ms=500`, with a diagnostic strict-barrier variant at `0`; verifier checks winner, reject basis, seq, settlement. | The last second of a jewellery/live-commerce auction where many users try to snipe. | Formal `2MLCX7WG`: 1000 sampling rows, 1000 unique bids, 285 accepted, 715 rejected, 1000 server POSTs, 1000 Redis Lua executions, 41 verifier gates PASS; sampling-log p99 23ms, max 28ms; PTS `startTimeTS` global span 1351ms, response `server_time_ms` global span 1348ms; per-agent spans 501/525ms. Diagnostic `TGLBX7GG`: strict barrier, p99 134ms, global span 1144ms. | M1 is request -> final `ENGINE_*`; accept and reject both count. `2MLCX7WG` proves the current 500ms per-agent burst profile meets p99 and correctness. | User gets final "accepted/outbid/too low" rather than pending guess; winner equals highest valid accepted bid. | PTS/JMeter multi-agent execution did not produce one global 1000-user 500ms interval; the honest claim is 500ms per pressure agent and measured 1.35s global span. If claiming a single global 500ms wall-clock interval, add one-agent/custom-generator evidence. | Formal S1 windowed profile PASS; global-rendezvous claim remains intentionally unclaimed. |
...
|---|---|---|---|---|---|
| L1-C1 `pts-1b-contention-burst-1000vu-1m.jmx` | 1000 one-shot final-second bid contention. | Not larger; it is S1's core asset. | S1. | Current review `2MLCX7WG` is a formal 500ms per-agent PASS, but it is not one global 500ms rendezvous across both pressure machines. | Keep as S1 source of truth; only tune load-generator/one-agent/custom-rendezvous if a reviewer demands a single global 500ms wall-clock claim. |
...
|---|---|---|---|
| S1 final-second contention | Yes | Official performance claim benefits from distributed source IPs and exportable charts. `2MLCX7WG` is the current formal PTS pass for the 500ms per-agent window; PTS is still not a global cross-agent rendezvous primitive. | Preserve `2MLCX7WG` as current S1 evidence; rerun only if changing the claim to one global 500ms interval. |
...
|---|---|---|
| S1 global-rendezvous gap | A current formal report exists with 1000 unique final `ENGINE_*` decisions, p99 23ms, and correctness PASS (`2MLCX7WG`), but it proves 500ms per pressure agent, not one global 500ms interval across both pressure machines. | Keep `2MLCX7WG` as formal S1; add one-agent/custom-generator evidence only if the public claim changes to global 500ms. |
