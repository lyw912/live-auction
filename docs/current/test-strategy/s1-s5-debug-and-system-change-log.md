...
| S3 mixed final-burst | integrated viewer join + bid + reads + fanout | PTS console counted 2995 viewer sampler chains, 998 bids, 499 readers with 100% success; service fanout `20965=7*2995`, DB bids=998, outbox=7 | PTS `S3 live fanout receive=2995` is sampler executions; raw fanout surface is service metric; DB stores business/outbox effects |
| S4 faults | live bidding during injected dependency failure | fault-window counters such as Kafka independent `bid_fault_window_decided_total=1000`, plus per-fault paused/decided/error assertions | a fault-window decision count proves overlap, not safety by itself; safety comes from RPO=0, no phantom accepts, no duplicate settlement, and convergence |
| S5 reconnect | stale-`last_seq` recovery after missed updates | k6 recovered counts, TTCS p99, 0 gaps/dups/truth mismatch; service `ws_reconnect`/`ws_recovered` distribution confirms backend recovery paths | recovered sessions are reconnect recovery attempts, not steady fanout messages; reconnect retries show fault turbulence rather than pass contamination |
