...
| Accepted settlement batching | `backend/internal/redisengine/engine.go` | Settlement worker now batches a contiguous prefix of same-auction, same-epoch, consecutive `ENGINE_ACCEPTED` Kafka messages in one transaction while still writing bids, auction events, outbox, idempotency, settlement rows, auction seq/price/winner, and checkpoint. Mixed, gap, stale, terminal, or replay cases fall back to the old per-message path. | `s2-capacity-accepted-ecs-20260604T150519` proved Redis decisions stayed fast at the 50/100/200/400/600 accepted-heavy stair, but async PG settlement/Kafka lag became the next bottleneck. The fix reduces per-accepted transaction overhead without changing the user-visible decision boundary or dropping audit. | `TestKafkaSettlementBatchesAcceptedPrefix`, `TestKafkaSettlementBatchesAcceptedPrefixBeforeReject`; post-fix independent-ECS rerun still required before calling S2-capacity full PASS |
...
| Accepted-heavy capacity stair | `s2-capacity-accepted-ecs-20260604T150519`: k6 exit 0, dropped 0, HTTP failed 0, 131,574 final decisions, 125,376 accepted, 6,198 rejected, p99 3.89ms, p99.9 7.47ms; PG settlement only about 61k and Kafka lag about 77,888 at collection. | Split verdict. Redis synchronous decision layer is strong; async settlement/outbox is the capacity knee. Added accepted contiguous-prefix batching and will rerun the same 600/s profile before any higher-pressure claim. |
